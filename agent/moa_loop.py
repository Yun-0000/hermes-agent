...
def _price_reference_response(
    ...
    """Price one advisor response at the slot's own rate. Never raises.

    ``call_llm`` rebuilds every provider payload into Chat Completions usage
    (``prompt_tokens`` / ``completion_tokens``). Normalizing that object with
    the slot's native Codex or Anthropic shape reads fields that are not
    there, so every bucket is 0 and advisor spend never reaches the session.
    The slot provider is still used for the dollar rate below.
    """
